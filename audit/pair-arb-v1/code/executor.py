...
class ArbExecutor:
    ...
    async def execute(self, opp: ArbOpportunity) -> ArbExecution:
        ...
        # Place both orders sequentially (httpx Client is not thread-safe,
        # asyncio.gather + to_thread causes "Request exception" on 2nd leg)
        try:
            up_result = await self._safe_place(up_quote, post_only)
            await asyncio.sleep(0.15)
            dn_result = await self._safe_place(dn_quote, post_only)
