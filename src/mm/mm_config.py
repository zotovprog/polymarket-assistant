...
class MMConfig:
    ...
    # ── Sizing ───────────────────────────────────────────────────
    order_size_usd: float = 5.0          # Smaller clip size to reduce per-fill risk
    min_order_size_usd: float = 2.0      # Below this, don't quote
    max_order_size_usd: float = 100.0

    # ── Inventory ────────────────────────────────────────────────
    max_inventory_shares: float = 30.0   # Allow larger one-sided buffer before forced liquidation
    max_net_delta_shares: float = 20.0   # Higher directional cap before skewing out of inventory
    skew_bps_per_unit: float = 6.0       # Softer skew to avoid overreacting to small imbalances

    # ── Requoting ────────────────────────────────────────────────
    requote_interval_sec: float = 2.0    # Slower quote refresh to reduce churn and cancellations
    requote_threshold_bps: float = 25.0  # Requote only on more meaningful fair-value moves
    ...
    # ── Window Management ────────────────────────────────────────
    close_window_sec: float = 90.0    # Start closing earlier to unwind inventory more gradually
    ...
    # ── Market Quality ─────────────────────────────────────────
    min_market_quality_score: float = 0.4   # Require stronger liquidity/quality before entry
